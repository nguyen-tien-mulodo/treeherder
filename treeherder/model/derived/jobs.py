...
into chunks of chunk_size size. Returns the number of result sets deleted"""
    ...
    def _load_job(self, job_datum, result_set_id):
        ...
        # insert the job unconditionally (we will just ignore the operation
        ...
        # FIXME: Do we need any logic to handle retry jobs? Or is this now
        # simpler now that we're only handling one job at a time?
        # if job_datum.get('result') == 'retry':
        #    retry_guid_root = get_guid_root(job_guid)
        #    Job.objects.filter(guid=retry_guid_root).exists()
